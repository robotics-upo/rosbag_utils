...
if (len(sys.argv) != 3):
  print 'Usage: {} <input bag> <output bag>'.format(sys.argv[0])
  sys.exit(-1)
        ...
        for topic, msg, t in bag.read_messages():
            if topic == '/tf' or topic == '/tf_static':
                for i in msg.transforms:
                    ...
                    if '//' in i.header.frame_id:
                        i.header.frame_id = i.header.frame_id.replace('//', '/')
                        changes += 1
                    if '//' in i.child_frame_id:
                        i.child_frame_id = i.child_frame_id.replace('//', '/')
                        changes += 1
                        
            if msg._has_header:
                    
                if '//' in msg.header.frame_id:
                    msg.header.frame_id = msg.header.frame_id.replace('//', '/')
                    changes += 1
...
print 'Corrected tfs. Number of changes: {}'.format(changes)